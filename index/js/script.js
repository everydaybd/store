...
async function loadAllReactions() {
    try {
        ...
        querySnapshot.forEach((doc) => {
            const data = doc.data();
            loadedReactions[doc.id] = {
                loveCount: data.loveCount || 0,
                lovedBy: data.lovedBy || []
            };
        });
        localStorage.setItem('everydaybd_local_reactions', JSON.stringify(loadedReactions));
    } catch (error) {
        console.warn("Firestore reactions fetch failed, falling back to LocalStorage:", error);
        const localData = localStorage.getItem('everydaybd_local_reactions');
        if (localData) {
            try {
                loadedReactions = JSON.parse(localData);
            } catch (e) {
                loadedReactions = {};
            }
        }
    ...
    if (allProducts && allProducts.length > 0) {
        allProducts.forEach(p => {
            updateReactionUI(p.ProductName);
        });
    }
}

function updateReactionUI(productName) {
    const productId = getProductId(productName);
    const reaction = loadedReactions[productId] || { loveCount: 0, lovedBy: [] };
    const currentUserId = auth.currentUser ? auth.currentUser.uid : null;
    const hasReacted = currentUserId ? reaction.lovedBy.includes(currentUserId) : false;

    const safeName = encodeURIComponent(productName);
    const cardButtons = document.querySelectorAll(`.love-btn[data-product="${safeName}"]`);
    cardButtons.forEach(btn => {
        btn.classList.toggle('loved', hasReacted);
        const heartIcon = btn.querySelector('i');
        if (heartIcon) {
            heartIcon.className = hasReacted ? 'fas fa-heart' : 'far fa-heart';
        }
        const countSpan = btn.querySelector('.love-count');
        if (countSpan) {
            countSpan.textContent = reaction.loveCount;
        }
    });

    const modalBtn = document.getElementById('pm-love-btn');
    if (modalBtn && decodeURIComponent(modalBtn.dataset.product || '') === productName) {
        modalBtn.classList.toggle('loved', hasReacted);
        const heartIcon = modalBtn.querySelector('i');
        if (heartIcon) {
            heartIcon.className = hasReacted ? 'fas fa-heart' : 'far fa-heart';
        }
        const countSpan = modalBtn.querySelector('.love-count');
        if (countSpan) {
            countSpan.textContent = reaction.loveCount;
        }
    }
...
async function toggleReaction(productName, triggerElement) {
    if (!auth.currentUser) {
        openAuthModal();
        return;
    }
    ...
    if (!loadedReactions[productId]) {
        loadedReactions[productId] = { loveCount: 0, lovedBy: [] };
    }

    const reaction = loadedReactions[productId];
    const hasReacted = reaction.lovedBy.includes(userId);

    if (hasReacted) {
        reaction.lovedBy = reaction.lovedBy.filter(id => id !== userId);
        reaction.loveCount = Math.max(0, reaction.loveCount - 1);
    } else {
        reaction.lovedBy.push(userId);
        reaction.loveCount += 1;
    }

    updateReactionUI(productName);

    try {
        const docRef = doc(db, "reactions", productId);
        const docSnap = await getDoc(docRef);

        if (!docSnap.exists()) {
            await setDoc(docRef, {
                loveCount: hasReacted ? 0 : 1,
                lovedBy: hasReacted ? [] : [userId]
            });
        } else {
            await updateDoc(docRef, {
                loveCount: increment(hasReacted ? -1 : 1),
                lovedBy: hasReacted ? arrayRemove(userId) : arrayUnion(userId)
            });
        }
        localStorage.setItem('everydaybd_local_reactions', JSON.stringify(loadedReactions));
    } catch (error) {
        console.warn("Firestore sync failed, caching update locally:", error);
        localStorage.setItem('everydaybd_local_reactions', JSON.stringify(loadedReactions));
    }
}
...
if (bnavProfile) {
    bnavProfile.addEventListener('click', (e) => {
        ...
        if (!auth.currentUser) {
            ...
            window.location.href = 'index/html/profile.html';
        }
    });
}
...
function openProductModal(product) {
    ...
    document.getElementById('pm-order-btn').href =
        `index/html/order.html?product=${encodeURIComponent(product.ProductName)}`;

    const modalLoveBtn = document.getElementById('pm-love-btn');
    if (modalLoveBtn) {
        modalLoveBtn.dataset.product = encodeURIComponent(product.ProductName);
    }
    updateReactionUI(product.ProductName);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createProductCard = (product, isFeatured = false, index = 0) => {
        const orderLink      = `index/html/order.html?product=${encodeURIComponent(product.ProductName)}`;
        ...
        const safeName = encodeURIComponent(product.ProductName);
        const loveBtnHTML = `
            <button class="love-btn" data-product="${safeName}" aria-label="Love product">
            ...
            <div class="${isFeatured ? 'featured-card reveal' : 'product-card reveal'}" style="--delay: ${delay}">
                ...
                <div class="product-info">
                    ...
                    <div class="product-title-row">
                        <h3 class="product-title pm-trigger" data-product="${encodeURIComponent(product.ProductName)}" style="cursor: pointer;">${product.ProductName}</h3>
                        ${loveBtnHTML}
                    </div>
    ...
    document.addEventListener('click', (e) => {
        if (e.target.closest('.love-btn')) return; // Ignore love btn trigger
    ...
    // Event delegation for love buttons click
    document.addEventListener('click', (e) => {
        const loveBtn = e.target.closest('.love-btn');
        if (loveBtn) {
            e.preventDefault();
            e.stopPropagation();
            const productName = decodeURIComponent(loveBtn.dataset.product);
            toggleReaction(productName, loveBtn);
            return;
        }

        const pmLoveBtn = e.target.closest('#pm-love-btn');
        if (pmLoveBtn) {
            e.preventDefault();
            const productName = decodeURIComponent(pmLoveBtn.dataset.product);
            toggleReaction(productName, pmLoveBtn);
        }
    });
    ...
    if (USE_MOCK_DATA) {
        setTimeout(() => { allProducts = mockProducts; filterProducts(); loadAllReactions(); }, 800);
    } else {
        fetch(GOOGLE_SHEET_CSV_URL)
            .then(res => { if (!res.ok) throw new Error('Network error'); return res.text(); })
            .then(csv => { 
                allProducts = parseCSV(csv); 
                localStorage.setItem('everydaybd_products', JSON.stringify(allProducts));
                filterProducts(); 
                loadAllReactions();
